...
export function AdminPluginInstancesCard({
  model,
}: {
  model: AdminPluginModel;
}) {
  const t = useT();
  const {
  ...
  return (
    <Card>
      <CardHeader className="border-b">
        ...
        <CardDescription>
          {t("admin.plugins.instances.description")}
        </CardDescription>
        <CardAction>
          <Dialog open={model.createOpen} onOpenChange={model.setCreateOpen}>
            <DialogTrigger render={<Button />}>
              {t("admin.plugins.instances.create")}
            </DialogTrigger>
            <DialogContent className="sm:max-w-lg">
              <form
                className="flex flex-col gap-4"
                onSubmit={model.onCreateInstance}
              >
                <DialogHeader>
                  <DialogTitle>
                    {t("admin.plugins.instances.createTitle")}
                  </DialogTitle>
                  <DialogDescription>
                ...
                <FieldGroup>
                  {model.createError ? (
                    <FieldError>{model.createError}</FieldError>
                  ) : null}
                  <Field>
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        model.setCreateName(event.target.value)
                      }
                    />
                  </Field>
                  <label
                    htmlFor="plugin-instance-enabled"
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        model.setCreateEnabled(value === true)
                      }
                    />
      ...
      <CardContent className="flex flex-col gap-4 pt-4">
        {model.instancesPending ? <Skeleton className="h-10 w-full" /> : null}
        {model.instances.length === 0 && !model.instancesPending ? (
          <p className="text-sm text-muted-foreground">
            {t("admin.plugins.instances.empty")}
          </p>
        ) : null}
        {model.instances.length > 0 ? (
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>
                  {t("admin.plugins.instances.column.name")}
                </TableHead>
                <TableHead>{t("admin.plugins.instances.column.id")}</TableHead>
                <TableHead>
                  {t("admin.plugins.instances.column.enabled")}
                </TableHead>
