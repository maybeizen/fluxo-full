...
4. Default-export a `defineServicePlugin` / `defineGatewayPlugin` / `definePanelPlugin` instance. Import **only** `@fluxo/forge` (plus the frontend `plugin-system` registration API for a panel widget). Do not import `@fluxo/db`, Prisma, `apps/api` services, or `theme-system`.
